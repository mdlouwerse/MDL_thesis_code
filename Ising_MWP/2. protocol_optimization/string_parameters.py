...
This file contains parameters for the 3x3 Ising model and its control-parameter
space. This file is loaded into all optimization scripts.
...
num_blocks = 4 # M, th number of blocks of fields in the system

# string parameters #
stringMin = -0.5*np.ones((num_blocks)) # initial CP value
stringMax = 0.5*np.ones((num_blocks)) # final CP value
R = 21 # number of points in string
duration = 1.0 # protocol duration (arbitrary for LR protocols)
delta_t = float(duration)/float(R-1) # time increment between subsequent string points
...
friction_directory = "{}/data/paper_version/friction_estimates".format(project_directory)
