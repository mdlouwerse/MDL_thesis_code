...
class quadcubic:
	...
	def sQuery1(self, query):
		...
		if query[0] < self.xIntMin or query[0] > self.xIntMax or query[1] < self.yIntMin or query[1] > self.yIntMax or query[2] < self.zIntMin or query[2] > self.zIntMax or query[3] < self.tIntMin or query[3] > self.tIntMax:
			...
			t = np.repeat(tvec,64)
			### interpolated values
			compx =  inner1d(self.alphax[:,self.queryInd], (x*y*z*t))
			compy =  inner1d(self.alphay[:,self.queryInd], (x*y*z*t))
			compz =  inner1d(self.alphaz[:,self.queryInd], (x*y*z*t))
			return np.array((compx, compy, compz))
	...
	def sQuery2(self, query):
		...
		if query[0] < self.xIntMin or query[0] > self.xIntMax or query[1] < self.yIntMin or query[1] > self.yIntMax or query[2] < self.zIntMin or query[2] > self.zIntMax or query[3] < self.tIntMin or query[3] > self.tIntMax:
			...
			###	interpolated values
			norm =  inner1d(self.alphan[:,self.queryInd], (x*y*z*t))
	...
	def rQuery1(self, query):
		...
		tz = np.transpose(tz)
		# Return components
		compsx = np.reshape(inner1d(tx, (x*y*z*t)), (N,1))
		compsy = np.reshape(inner1d(ty, (x*y*z*t)), (N,1))
		compsz = np.reshape(inner1d(tz, (x*y*z*t)), (N,1))
		return np.hstack((compsx, compsy, compsz))
	...
	def rQuery2(self, query):
		...
		tn = np.transpose(tn)
		# Return magnitude
		norms = np.reshape(inner1d(tn, (x*y*z*t)), (N,1))
		# Return gradient
		grads = np.transpose(np.array([(inner1d(tn, (xx*y*z*t))/self.hx), (inner1d(tn, (x*yy*z*t))/self.hy), (inner1d(tn, (x*y*zz*t))/self.hz), (inner1d(tn, (x*y*z*tt))/self.ht) ]))
		return norms, grads
